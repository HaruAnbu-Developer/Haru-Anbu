...
### 해결 방안
**AI 목소리 복제 기술**로 자녀의 목소리를 학습하여, 어르신께 **실제 자녀가 전화한 것 같은 경험**을 제공합니다.
```
...
| **Telephony** | Twilio Voice API | 9.14.1 |
| **Real-time** | WebSocket | - |
...
| **LLM** | Gemma-2-9B (GGUF) | 대화 생성 |
| **TTS** | Coqui XTTS v2 | 목소리 복제 |
| **Framework** | FastAPI | API 서버 |
| **Communication** | gRPC Python | 1.76.0 |
...
#### 세션 조회
```http
GET /api/calls/sessions/{sessionId}
```
...
#### 목소리 등록 시작
```http
POST /voice/register/{userId}
```

#### 목소리 준비 (통화 전)
```http
POST /voice/prepare/{userId}
```

#### 목소리 해제 (통화 후)
```http
POST /voice/release/{userId}
```
